...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedAcc(max: Tweet): Tweet = {
    if (elem.retweets > max.retweets) left.mostRetweetedAcc(right.mostRetweetedAcc(elem))
    else left.mostRetweetedAcc(right.mostRetweetedAcc(max))
  }

  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val left_and_right = left.filterAcc(p, right.filterAcc(p, acc))
    if (p(elem)) left_and_right.incl(elem)
    else left_and_right
  }
